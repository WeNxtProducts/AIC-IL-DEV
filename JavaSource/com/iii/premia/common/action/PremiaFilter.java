...
public class PremiaFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		String applicationContextURL = "http://" + request.getLocalAddr() + ":" + request.getServerPort()
		...
		System.out.println("previousURL        " + previousURL + "      currentURL       " + currentURL + "        "
				+ applicationContextURL);

		/*
		 * System.out.println("request.getLocalAddr()      "
		 * +request.getLocalAddr()); // Returns: 127.0.0.1 System.out.println(
		 * "request.getLocalName()      "+request.getLocalName()); // Returns:
		 * localhost System.out.println("request.getServerName()      "
		 * +request.getServerName()); // Returns: localhost System.out.println(
		 * "request.getServerPort()      "+request.getServerPort()); // Returns:
		 * 8080 System.out.println("request.getContextPath()      "
		 * +request.getContextPath()); // Returns: /MyApp System.out.println(
		 ...
		 * 
		 * if((currentURL != null &&
		 * currentURL.toString().contains("login.xhtml")) ||
				...
				|| (currentURL != null && currentURL.toString().contains("browserNotSupported.xhtml")))// end
		{
			...
			try {
				...
				if (previousURL == null) {
					...
					// new LoginService().invalidateSessionAction(request,
					// context);
					...
					/*
					 * Added by saritha on 05-Mar-2017 for ssp call id
					 * ZBILQC-1719135
					 */
					/* response.sendRedirect(applicationContextURL); */
				} else {
					/*
					 * Added by Ram on 03/02/2017 for Handling DBsession
					 * variables
					 */
					HttpSession session = request.getSession();
					if (session != null) {
						...
						if (clientId != null) {
							System.out.println("------------premiaFilter-Connection1--------");

							HashMap param = new HashMap();
							ServletContext context = request.getSession().getServletContext();
							Connection con = ((SimpleConnectionAgent) context.getAttribute("connectionAgent"))
	...
	public void setDBvalues(HttpSession session, Connection conn) {
		...
		OracleParameter param9 = new OracleParameter("in9", "STRING", "IN", null);
		OracleParameter param10 = new OracleParameter("in10", "STRING", "IN", "P11JLIFEJAVA");
		...
		try {
			...
			procHandler.execute(paramList, conn, "PCOPK_SYS_VARS.P_SET_PARA_VALUES");
			/* end */
		} catch (Exception e) {
			e.printStackTrace();
		}
