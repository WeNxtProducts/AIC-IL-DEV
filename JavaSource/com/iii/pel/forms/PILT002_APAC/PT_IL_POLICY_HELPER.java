...
public class PT_IL_POLICY_HELPER extends CommonAction {
			...
			if (M_DUM_VALUE_AGE == 1) {
				...
				if (list1 != null && !list1.isEmpty()) {
					if (list1.get(0) != null) {
						...
						/*Newly added by pidugu raj dt: 29-11-2018 as per sivaraman suggestion
						 * Purpose: If policy startdate changed back to years then policy term also should be changed*/
						
						
						ResultSet resultSetNew = null;
						String queryForPolPeriod = "SELECT PLAN_MAX_AGE_ON_MAT,PLAN_MIN_AGE_ON_MAT,PLAN_AGE_TERM_FLAG"
								+ " PLAN_AGE_FROM,PLAN_AGE_TO,PLAN_PERIOD_FROM, PLAN_PERIOD_TO, PLAN_TYPE "
								+ " FROM PM_IL_PLAN WHERE  PLAN_CODE = ?  AND PLAN_FRZ_FLAG = 'N'";
						
						/*Modified By Janani dt : 04.07.2019
						Purpose :- For G01 Pol period calculation is not required for Sarwa suggested by Vijay
						           Need to check with Sivaram*/
						//if(pt_il_policy_bean.getPOL_PROD_CODE() != null && (pt_il_policy_bean.getPOL_PROD_CODE().equalsIgnoreCase("G01")|| pt_il_policy_bean.getPOL_PROD_CODE().equalsIgnoreCase("G02")) &&!pt_il_policy_bean.getPOL_PROD_CODE().isEmpty()){
						if(pt_il_policy_bean.getPOL_PROD_CODE() != null && (pt_il_policy_bean.getPOL_PROD_CODE().equalsIgnoreCase("G02")) &&!pt_il_policy_bean.getPOL_PROD_CODE().isEmpty()){	
						
						/*End*/	
						resultSetNew = handler.executeSelectStatement(queryForPolPeriod, CommonUtils.getConnection(),new Object[]{(pt_il_policy_bean.getPOL_PROD_CODE())});
						if(resultSetNew.next()){
							int pLAN_MAX_AGE_ON_MAT = resultSetNew.getInt(1);
							pt_il_policy_bean.setPOL_PERIOD(pLAN_MAX_AGE_ON_MAT-pt_il_policy_bean.getPOL_JOINT_LIFE_AGE());
							
							
							ResultSet resultSetNew2 = null;
							String queryForPremTerm = "select PROD_PREM_PAYING_YRS from PM_IL_PRODUCT where prod_code = ?";
							resultSetNew2 = handler.executeSelectStatement(queryForPremTerm, CommonUtils.getConnection(),new Object[]{(pt_il_policy_bean.getPOL_PROD_CODE())});
							//Check the value based on variable or as it is
							if(resultSetNew2.next()){
								String prodPremPayingYears = resultSetNew2.getString(1);
								if(prodPremPayingYears.equalsIgnoreCase("V")){
									//don't reset the pol period with premium term
								}else{
						...
						/*End newly added by pidugu raj dt: 29-11-2018 as per sivaraman suggestion
						 * Purpose: If policy startdate changed back to years then policy term also should be changed*/
	...
	public Integer POL_PREM_PAY_YRS_WHEN_VALIDATE_ITEM(
		...
		try {
			...
			if (pt_il_policy_bean.getPOL_FIRST_INST_DT() != null
					&& pt_il_policy_bean.getPOL_START_DT() != null) {/*Modified by pidugu raj dt: 18-09-2018 as suggested by sivaraman for P11JLIFE_ZBDEV 
				 * 
				 * if (pt_il_policy_bean.getPOL_FIRST_INST_DT().before(
						pt_il_policy_bean.getPOL_START_DT())) {
					throw new Exception(Messages.getString(
							PELConstants.pelErrorMessagePath, "71035"));
				}End Modified by pidugu raj  */
				
				/*Modified by pidugu raj dt: 18-09-2018 as suggested by sivaraman for P11JLIFE_ZBDEV */
				Date pOL_START_DT =pt_il_policy_bean.getPOL_START_DT();
				DateFormat dateformate = new SimpleDateFormat("dd/MM/yyyy");
				String poldate = dateformate.format(pOL_START_DT);
				Date trunc_Pol_Start_Date = dateformate.parse(poldate);
				
				
				if (pt_il_policy_bean.getPOL_FIRST_INST_DT().before(
						trunc_Pol_Start_Date)) {
					throw new Exception(Messages.getString(
							PELConstants.pelErrorMessagePath, "71035"));
				}
				/*End Modified by pidugu raj  dt: 18-09-2018*/}
	...
	public void POL_FC_SUM_ASSURED_WHEN_VALIDATE_ITEM(
		...
		/*Commented and modified by pidugu raj dt: 17-12-2018  as suggested by sivaraman and vijay for lc validation to handle instead of fc
		 * 		pValList_sa = new PKG_PILT002_APAC().P_MIN_MAX_SA_VALID(
			...
			pValList_sa = new PKG_PILT002_APAC().P_MIN_MAX_SA_VALID(
					...
					pt_il_policy_bean.getPOL_PLAN_CODE(), 
					null);
			/*End Commented and modified by pidugu raj dt: 17-12-2018  as suggested by sivaraman and vijay for lc validation to handle instead of fc*/
			...
			
			{
				...
				if ((CommonUtils.nvl(currValue, 0.0) + CommonUtils.nvl(
					...
					System.out.println("##POL_LC_SUM_ASSURED_USD_71043##"+ pt_il_policy_bean.getPOL_LC_SUM_ASSURED());
					/*End*/
			...
			if (currValue != null) {
				...
				//added by gopi on 28/12/2021
				if(pt_il_policy_bean.getPOL_SA_CURR_CODE().equalsIgnoreCase("USD")
						&& pt_il_policy_bean.getPOL_CUST_CURR_CODE().equalsIgnoreCase("USD")){
					Double lc_sum_assured=0.0;
					 lc_sum_assured=currValue* pt_il_policy_bean.getPOL_SA_EXCH_RATE();
						DecimalFormat myFormatter = new DecimalFormat("#");
						String value = myFormatter.format(lc_sum_assured);
						System.out.println("##POL_LC_SUM_ASSURED_STRING_USD##"+ value);
					pt_il_policy_bean.setPOL_LC_SUM_ASSURED(Double.parseDouble(value));
					System.out.println("##POL_LC_SUM_ASSURED_USD##"+ pt_il_policy_bean.getPOL_LC_SUM_ASSURED());
				}else{
			...
			if (pValListStr != null && !pValListStr.isEmpty()) {
				if (pValListStr.get(0) != null) {
					...
					System.out.println("##POL_LC_SUM_ASSURED_USD_R##"+ pt_il_policy_bean.getPOL_LC_SUM_ASSURED());
				}
			}
				...
				if (pValList != null && !pValList.isEmpty()) {
					...
					if (pValList.get(6) != null) {
						pt_il_policy_bean
								.setPOL_LC_SUM_ASSURED((Double) pValList.get(6)
										.getValueObject());
						System.out.println("##POL_LC_SUM_ASSURED_USD_LCAL##"+ pt_il_policy_bean.getPOL_LC_SUM_ASSURED());
					}
				}
			}
...
/*Modified by Ram on 14/02/2017 for update Rider sumassured values
	...
	public void POL_FRZ_FLAG_WHEN_VALIDATE_ITEM(
		...
		/*Newly added by pidugu raj dt: 29-09-2020 as suggeseted by kk for Alliance IL*/
		if(pt_il_policy_bean.getPOL_PROD_CODE().equalsIgnoreCase("EDU00001") || pt_il_policy_bean.getPOL_PROD_CODE().equalsIgnoreCase("LPLAN001")){
		P_MAX_SUMASSURED_LPLAN_EPLAN(pt_il_policy_bean.getPOL_CUST_CODE(),String.valueOf(pt_il_policy_bean.getPOL_SYS_ID()));
		}
		/*End Newly added by pidugu raj dt: 29-09-2020 as suggeseted by kk for Alliance IL*/
	...
	public void POL_END_EFF_FROM_DT_WHEN_VALIDATE_ITEM(
		...
		try {
			...
			NO_OF_DAYS = CommonUtils.subtractDate(pt_il_policy_bean
					.getPOL_END_DT(), pt_il_policy_bean.getPOL_START_DT());
			/*as suggested by ajoy commanted by gopi on 30/10/2018*/
			...
			}*///commented patch added from orient
 ...
 * pValList = pkg_pilt002_apac.POL_END_EFF_FROM_DT_W_V_I(
			...
			if (pValList != null && pValList.size() > 0) {
				...
				if (pValList.get(21) != null) {
					...
					/*uncommented & modified by Ameen on 27-07-2018 for KIC To change pol_period KICLIFEQC-1759635 - 2,6*/
					   		...
					   		/*Added by Janani on 12.11.2018 for KIC 
					   		issue  : Policy Term getting changed when Freeze flag is changed to No*/
					   		
					   		/*Modified by pidugu raj dt: 19-11-2018 for term resetting issue 
					   		 * if(pt_il_policy_bean.getPOL_DS_TYPE().equals("2") && PROD_CALC_TERM_YN.equalsIgnoreCase("n") && 
									 "KIC/".equalsIgnoreCase(CommonUtils.getGlobalVariable("GLOBAL.SITE_CODE")))
							 {*/
					   		
					   		if(pt_il_policy_bean.getPOL_DS_TYPE().equals("1") || pt_il_policy_bean.getPOL_DS_TYPE().equals("2") && PROD_CALC_TERM_YN.equalsIgnoreCase("n") && 
									 "KIC/".equalsIgnoreCase(CommonUtils.getGlobalVariable("GLOBAL.SITE_CODE")))
							 {
					   		/*End Modified by pidugu raj dt: 19-11-2018*/
								 
								 System.out.println("prod calc term is N");
								 
								 compositeAction.getPT_IL_POLICY_ACTION_BEAN().setPOL_PERIOD_CHNG(true);
								 
								 System.out.println("getPOL_PERIOD_CHNG          "+compositeAction.getPT_IL_POLICY_ACTION_BEAN().isPOL_PERIOD_CHNG());
								 
								 
								 
								 if(pt_il_policy_bean.getPOL_PERIOD() !=null && pt_il_policy_bean.getPOL_PERIOD() !=  Integer.parseInt(pValList.get(21))){
	...
	public void POL_ASSRD_REF_ID1_WHEN_VALIDATE_ITEM(
		...
		/*String query12 = "SELECT CUST_CODE,CUST_DOB  FROM   PM_CUSTOMER  WHERE UPPER(CUST_CODE) LIKE UPPER('IND%') AND"
				+ " NVL(CUST_REF_ID1,'X') = NVL(? ,'X')";*/
		/*Modified by Sivarajan on 14/08/2019 for Sarwa il*/
		/*
		...
		/*
		 * modified by Ameen after discussing with Sivarajan that below query can be used for prem pay code
		 * String query12 = "SELECT CUST_CODE,CUST_DOB  FROM   PM_CUSTOMER  WHERE"
				+ " NVL(CUST_REF_ID1,'X') = NVL(? ,'X')";*/
		String query12 = "SELECT CUST_CODE,CUST_DOB  FROM   PM_CUSTOMER  WHERE"
				+ " NVL(CUST_REF_ID1,'X') = NVL(? ,'X')";
	...
	public void WHEN_CREATE_RECORD(PILT002_APAC_COMPOSITE_ACTION compositeAction)
		...
		/*Newly added by pidugu raj dt: 16-09-2020 as suggested by kk and sitrarasan*/
		...
		Connection connection = null;
		/*Commented and modified by pidugu raj dt: 16-09-2020 to call one more query for product default currrency as suggested
		 * by kk
		...
		try {
			...
			if (policyAction.isINSERT_ALLOWED() == true) {
				...
				/*Commented and modified by pidugu raj for Alliance IL as suggested by KK dt: 15-09-2020
				 * policyBean.setPOL_SA_CURR_CODE(dummy_bean.getUI_M_BASE_CURR());*/
				/*if(policyBean.getPOL_PROD_CODE().equalsIgnoreCase("KMAN001")){
					policyBean.setPOL_SA_CURR_CODE("USD");
				}else{
					policyBean.setPOL_SA_CURR_CODE(dummy_bean.getUI_M_BASE_CURR());
				}*/
				String query7 = "SELECT PROD_DEFAULT_CURR_CODE FROM PM_IL_PRODUCT WHERE PROD_CODE = ?";
				object7 = new Object[] { policyBean.getPOL_PROD_CODE() };
				rs7 = handler.executeSelectStatement(query7, connection, object7);
				if (rs7.next()) {

					policyBean.setPOL_SA_CURR_CODE(rs7.getString(1));
					policyBean.setPOL_CUST_CURR_CODE(rs7.getString(1));
				}
				//end
				...
				/*if(policyBean.getPOL_PROD_CODE().equalsIgnoreCase("KMAN001")){
					policyBean.setPOL_CUST_CURR_CODE("USD");
				}else{
				policyBean.setPOL_CUST_CURR_CODE(dummy_bean.getUI_M_BASE_CURR());
				}*/
		...
		try {
				...
				if ("1".equals(policyBean.getPOL_DS_TYPE())) {
					...
					if (3 == dummyBean.getUI_M_CALC_METHOD()) {
						...
						if ("A".equals(CommonUtils.nvl(policyBean
							...
							if (!"Q".equals(CommonUtils.getGlobalVariable(
									"GLOBAL.M_PARAM_13"))
									&& "1"
											.equals(CommonUtils
													.getGlobalVariable("GLOBAL.M_PARAM_11"))) {
			...
			if (dummyBean.getUI_M_POL_PLAN_TYPE() != "D") {
				if ("P".equals(dummyBean.getUI_M_COVER_SA_CALC())
					...
					/*if condition handled by Ameen for SARWA retirement product sugg. by Sivaram*/
					if("U".equalsIgnoreCase(policyBean.getPOL_UL_PREM_CALC())){
						compositeAction.getPT_IL_POLICY_ACTION_BEAN().getCOMP_POL_FC_MODAL_PREM().setDisabled(false);
...
//				policyAction.getCOMP_POL_FC_MODAL_PREM().setRequired(true);
			...
			}else
			{
				...
				*/
				...
				policyAction.getCOMP_POL_ANN_BNF_AMT().setDisabled(true);
				/*if condition handled by Ameen on 31-10-2019 for SARWA Retirement product sugg. by Sivaram*/
				if("U".equalsIgnoreCase(policyBean.getPOL_UL_PREM_CALC())){
					policyAction.getCOMP_POL_FC_MODAL_PREM().setDisabled(false);
					policyAction.getCOMP_POL_FC_MODAL_PREM().setRequired(true);
				}else{
	 ...
	 /*Added by saritha on 19-12-2016 RM018T -DEV_IL_ZB Life_015-Lapsation and commission clawback 2*/
		...
		/*Added by Janani on 14.07.2017 for ZBILQC-1731112 */
		 ...
		 public ArrayList<OracleParameter> callP_FUNERAL_SUB_PLAN_DFLT_SA(String  POL_PROD_CODE,String Sub_Plan_Code) throws Exception {
				...
				try {
					...
					parameterList.add(param3);
					OracleParameter param4 = new OracleParameter("OUT2", OracleParameter.DATA_TYPE.STRING,	OracleParameter.DIRECTION_TYPE.OUT, "");
		...
		/*Added by Ameen on 26-07-2018 for TOI */ 
		 ...
		 /*Added by pidugu raj dt: 27-08-2018 for KICLIFEQC-1762428*/
			...
			/*Newly added by pidugu raj dt:29-09-2020 as suggested by kuzhandaivel for Alliance IL*/
			private void P_MAX_SUMASSURED_LPLAN_EPLAN(String P_CUST_CODE,String POL_SYS_ID) throws Exception {
				...
				try {
					...
					parameterList = new ArrayList<OracleParameter>();
					outputList = new ArrayList<OracleParameter>();
					String P_SUCCESS_MSG = null;
					OracleParameter param1 = new OracleParameter("IN1", "STRING", "IN",P_CUST_CODE);
					parameterList.add(param1);
					OracleParameter param2 = new OracleParameter("IN2", "LONG", "IN",POL_SYS_ID);
					parameterList.add(param2);
					OracleParameter param3 = new OracleParameter("OUT3", "STRING", "OUT",P_SUCCESS_MSG);
					parameterList.add(param3);
					
					OracleProcedureHandler procHandler = new OracleProcedureHandler();
					outputList = procHandler.execute(parameterList, connection,
							"P_MAX_SUMASSURED_LPLAN_EPLAN");
					Iterator<OracleParameter> iterator = outputList.iterator();
					while (iterator.hasNext()) {
						OracleParameter oracleParameter = iterator.next();
						String value=oracleParameter.getValue();
						if(value.equalsIgnoreCase("Y")){
							throw new Exception("Sum Assured for the customer Exceeds 40000000");
						}
			...
			/*End Newly added by pidugu raj dt:29-09-2020 as suggested by kuzhandaivel for Alliance IL*/
