			...
			try {	
				...
				if(CommonUtils.nvl(procedureReqd,"X").equals("Y"))
				{
					...
					*/
					...
					url=genReport(IP_REP_INFO_BEAN,null,null,"S",rep_template_name,null,con,moduleId,REP_KEY_NO,REP_ID,"S",viewtype);
					/*End*/
					System.out.println(url);
					response.sendRedirect(url);
				}
				else
				{
					...
					if(REP_ID.equals("AICREP014") || REP_ID.equals("AICREP003")){
					moduleId="PILT004";
					REPORT_TYPE = REP_ID;
					}
					String rep_template_name = template_name(moduleId,POL_APPRV_STATUS,REPORT_TYPE,con);
					/* Modified by ganesh on 12-05-2017*/
					/*Modified by Ram on 02/03/2017 for Mail purpose*/
					if(REP_ID.equals("AICREP010") ||REP_ID.equals("AICREP018") ||REP_ID.equals("AICREP010") ||REP_ID.equals("AICREP010")){
						 url=genReport(IP_REP_INFO_BEAN,null,null,"JAS",rep_template_name,null,con,moduleId,REP_KEY_NO,REP_ID,"JAS",viewtype);
					} else {
						url=genReport(IP_REP_INFO_BEAN,null,null,"S",rep_template_name,null,con,moduleId,REP_KEY_NO,REP_ID,"S",viewtype);
					}
					/*End*/
...
/*Modified by ganesh on 12-05-2017 */
			...
			,String REP_KEY_NO,String reportTemplateRepID,String Type,String viewtype)
	{
		...
		try
		{
			...
			} else if("JAS".equalsIgnoreCase(reportCategory)){
				url=resourceBundle.getString("Jasper_newServeletUrl")+"reportTemplateRepID="+reportTemplateRepID+"&REP_KEY_NO="+REP_KEY_NO;
			}
	...
	public String template_name(String moduleId,String POL_APPRV_STATUS,String reportType,Connection con)throws Exception
	{
		...
		else
		{
			...
			String query ="SELECT PMRTH_REP_TEMP_NAME_ID FROM pm_rep_tab_hdr WHERE PMRTH_MODULE_ID = ? AND PMRTH_REP_CATG = 'L' AND PMRTH_REP_ID = ?";

			resultSet = new CRUDHandler().executeSelectStatement(
					query, connection,new Object[]{moduleId, reportType});
