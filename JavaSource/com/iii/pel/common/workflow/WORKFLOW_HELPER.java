...
public class WORKFLOW_HELPER {
	...
	public List<PGIM_WORKFLOW_BUCKET> getWorkFlowBucket() throws Exception {
		...
		try {
			System.out.println("------------workflow-Connection1--------");
			...
			String query1 = "SELECT "
					...
					+ "Count(*) As TranCount " + "FROM PGIW_WF_TRAN_HDR HDR, PGIM_WORKFLOW_BUCKET WB "
					+ "WHERE WB.PWB_ENABLE_YN = '1' " + "AND WB.PWB_MAIN_BUKT = HDR.PWTH_PROCESS_ID "
					+ "AND WB.PWB_CODE = HDR.PWTH_PWB_CODE " + "AND NVL(WB.PWB_MAIL_ONLY_YN, '0') = '0' "
					+ "AND HDR.PWTH_USER_ID LIKE ? "
					// + "AND WB.PWB_CODE IN
					// ('POAIL','PAIL','POIL','CIIL','CEIIL','CEL','CEIL') "
					+ "GROUP BY "
					/* + "HDR.PWTH_DEPT_CODE, " */
					/* + "HDR.PWTH_FLEX_10, " */
					+ "HDR.PWTH_PROCESS_ID, HDR.PWTH_WORKSTEP_ID ," + "HDR.PWTH_PWB_CODE, " + "WB.PWB_DESC, "
					+ "WB.PWB_SR_NO " + "ORDER BY "
					/* + "HDR.PWTH_DEPT_CODE, " */
					+ "HDR.PWTH_PROCESS_ID, WB.PWB_SR_NO";

			ResultSet resultusergroup = handler.executeSelectStatement(
					"SELECT USER_GROUP_ID FROM MENU_USER WHERE USER_ID=?", connection,
					new Object[] { CommonUtils.getGlobalVariable("GLOBAL.M_USER_ID") });
			String USER_ID = null;
			while (resultusergroup.next()) {
				USER_ID = resultusergroup.getString(1);
			}
			result = handler.executeSelectStatement(query1.toString(), connection,
					new Object[] { "%" + CommonUtils.getGlobalVariable("GLOBAL.M_USER_ID") + "%" });
			/* End */
			...
			if (!connection.isClosed() && connection != null) {
				System.out.println("----------------workflow-----connclose2");
	...
	public List<PGIM_WORKFLOW_BUCKET> getWorkFlowUnAssignedBucket() throws Exception {
		...
		try {
			connection = CommonUtils.getConnection();
			System.out.println("------------workflow-Connection2--------");
			...
			String query1 = "SELECT "
					 ...
					 * "HDR.PWTH_DEPT_CODE,HDR.PWTH_FLEX_10 AS DEPT_DESCRIPTION, "
					 */
					+ "HDR.PWTH_PROCESS_ID,HDR.PWTH_PWB_CODE,WB.PWB_DESC,WB.PWB_SR_NO, " + "Count(*) As TranCount "
					+ "FROM PGIW_WF_TRAN_HDR HDR, PGIM_WORKFLOW_BUCKET WB " + "WHERE WB.PWB_ENABLE_YN = '1' "
					+ "AND WB.PWB_MAIN_BUKT = HDR.PWTH_PROCESS_ID " + "AND WB.PWB_CODE = HDR.PWTH_PWB_CODE "
					+ "AND NVL(WB.PWB_MAIL_ONLY_YN, '0') = '0' " + "AND HDR.PWTH_USER_ID IS NULL "
					+ "AND HDR.PWTH_TARGET_USGRP IN (?,?) " + "AND PWTH_TRAN_PROG_CODE NOT LIKE 'PGL%' " + "GROUP BY "
					/* + "HDR.PWTH_DEPT_CODE, " */
					/* + "HDR.PWTH_FLEX_10, " */
					+ "HDR.PWTH_PROCESS_ID, " + "HDR.PWTH_PWB_CODE, " + "WB.PWB_DESC, " + "WB.PWB_SR_NO " + "ORDER BY "
					/* + "HDR.PWTH_DEPT_CODE, " */
					+ "HDR.PWTH_PROCESS_ID, WB.PWB_SR_NO";

			String query2 = "SELECT USER_GROUP_ID FROM MENU_USER_REPT_HIER, MENU_USER " + "WHERE RH_REPORT_TO = ? "
					+ "AND RH_USER_ID = USER_ID " + "AND (TRUNC(SYSDATE) BETWEEN RH_EFF_DT_FM AND RH_EFF_DT_TO)";

			ResultSet rs = handler.executeSelectStatement(query2, connection,
					new Object[] { CommonUtils.getGlobalVariable("GLOBAL.M_USER_ID") });
			...
			result = handler.executeSelectStatement(query1.toString(), connection,
					new Object[] { USER_GRP, REPORT_USER_GRP });
			...
			CommonUtils.closeCursor(result1);
			if (!connection.isClosed() && connection != null) {
				System.out.println("----------------workflow-conclose----connclose3");
				connection.close();
			}
		}
		return headerList;
	...
	public List<Pgiw_wf_tran_dtls> getWorkFlowDetails(String code, String userId, String deptCode) throws Exception {
		...
		try {
			connection = CommonUtils.getConnection();
			System.out.println("------------workflow-Connection3--------");
			...
			String queryString = "SELECT PWTH_SYS_ID, PWTH_ESC_LVL_DONE, PWTH_CR_UID, PWTH_CR_DT FROM "
					...
					+ "FROM PGIW_WF_TRAN_DTLS DTLS, PGIW_WF_TRAN_HDR HDR, " + "MENU_USER_COMP_DIVN MUCD "
					+ "WHERE HDR.PWTH_SYS_ID = DTLS.PWTD_PWTH_SYS_ID " + "AND HDR.PWTH_PWB_CODE = '" + code + "' "
					+ "AND MUCD.MUCD_USER_ID = '" + userId + "' " + "AND HDR.PWTH_USER_ID = MUCD.MUCD_USER_ID "
			...
			result2 = handler.executeSelectStatement(query_code.toString(), connection);
			...
			while (result.next()) {
				...
				String query = "select * from pgiw_wf_tran_dtls where pwtd_pwth_sys_id='" + sysId
						+ "' ORDER BY PWTD_WF_CHK_YN";
				result1 = handler.executeSelectStatement(query.toString(), connection);
				...
				if ("UNDERWRITING".equalsIgnoreCase(Main_bkt_code)) {
					/*
					 * String sDate1=s.get(0); Date date1=new
					 * SimpleDateFormat("dd-MMM-yyyy").parse(sDate1); String
					 * dates = new SimpleDateFormat("dd/MM/yy").format(date1);
					 * 
				...
				if ("FINANCE".equalsIgnoreCase(Main_bkt_code)) {
					/*
					 * String sDate1=s.get(0); if(sDate1!=null){ Date date1=new
					 * SimpleDateFormat("dd-MMM-yyyy").parse(sDate1); String
					 * dates = new SimpleDateFormat("dd/MM/yy").format(date1);
					 * Pgiw_wf_tran_dtls.setCreateUserDate(dates); }
					 */
				...
				if ("OT_PC".equalsIgnoreCase(Main_bkt_code) || "OCD_PC".equalsIgnoreCase(Main_bkt_code)
						|| "OT_SUR".equalsIgnoreCase(Main_bkt_code) || "OCD_SUR".equalsIgnoreCase(Main_bkt_code)
	...
	public List<Pgiw_wf_tran_dtls> getWorkFlowUnAssignedDetails(String code, String userId, String deptCode)
			throws Exception {
		List<String> s = new ArrayList<String>();
		ResultSet result = null;
		ResultSet result1 = null;
		...
		try {
			connection = CommonUtils.getConnection();
			System.out.println("------------workflow-Connection4--------");
			...
			String queryString = "SELECT PWTH_SYS_ID, PWTH_ESC_LVL_DONE, PWTH_CR_UID, PWTH_CR_DT,PWTH_USER_ID FROM "
					...
					+ "FROM PGIW_WF_TRAN_DTLS DTLS, PGIW_WF_TRAN_HDR HDR, " + "MENU_USER_COMP_DIVN MUCD "
					+ "WHERE HDR.PWTH_SYS_ID = DTLS.PWTD_PWTH_SYS_ID " + "AND HDR.PWTH_PWB_CODE = '" + code + "' "
					+ "AND HDR.PWTH_USER_ID IS NULL " + "AND HDR.PWTH_TARGET_USGRP IN (?,?) "
					+ "ORDER BY HDR.PWTH_SYS_ID ) D "
			...
			CommonUtils.closeCursor(result1);
			if (!connection.isClosed() && connection != null) {
				System.out.print("----------------workflow-----connclose1");
				connection.close();
			}
		}
		return headerList;
	...
	public String getOutcome(String PWB_WORKSTEP_ID) {
		String outCome = null;
		try {
			System.out.println("------------workflow-Connection5--------");

			String query = "SELECT PWB_OUTCOME_DET FROM PGIM_WORKFLOW_BUCKET WHERE PWB_CODE=?";
			ResultSet outComeRS = new CRUDHandler().executeSelectStatement(query, CommonUtils.getConnection(),
					new Object[] { PWB_WORKSTEP_ID });
	...
	public String getToBeOutcome(String PWB_WORKSTEP_ID) {
		String outCome = null;
		try {
			System.out.println("------------workflow-Connection6--------");
	...
	public Date getDueDate(String code, Date taskAssignedDate, String userId) {
		...
		try {
			...
			String C2 = "SELECT USER_GROUP_ID, MUCD_DIVN_CODE FROM MENU_USER, MENU_USER_COMP_DIVN WHERE USER_ID = MUCD_USER_ID AND USER_ID = '"
					+ userId + "'";
			...
			if (PWMU_ESC_LMT_TYP != null && PWMU_ESC_LMT_TYP.equalsIgnoreCase("D")) {
				...
				while (ESC_NEW > 0) {
					...
					String C3 = " SELECT COUNT(PWHM_DATE) COUNT_NUM" + " FROM PGIM_WF_HOLIDAY_MASTER"
							+ " WHERE PWHM_DATE = TO_DATE('" + next_day1 + "', 'DD/MM/RRRR')" + " AND PWHM_USER_GRP ="
							+ " DECODE(PWHM_USER_GRP,'ALL','ALL'," + " LTRIM(RTRIM(REPLACE('" + USER_GROUP_ID
							+ "', '''', NULL))))" + " AND PWHM_BRANCH_CODE = '" + USER_BRANCH + "'";
				...
				if (hours <= DIVN_WORK_TIME_TO && minutes == 0) {
					...
					while (time >= 0) {
						...
						String C3 = " SELECT COUNT(PWHM_DATE) COUNT_NUM" + " FROM PGIM_WF_HOLIDAY_MASTER"
								...
								+ " AND PWHM_USER_GRP =" + " DECODE(PWHM_USER_GRP,'ALL','ALL',"
